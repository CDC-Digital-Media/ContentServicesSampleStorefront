...
//VIDEO EMBED IS THE EMBED CODE (TAB) AND TABS SETUP

(function ($) {
    var PLUGIN_NAME = 'videoEmbed';
    ...
    $.fn[PLUGIN_NAME] = function (options) {
        ...
        function main() {
            ...
            var $divPreview = $("<div class='videoPreviewPane'>");
        ...
        function setupTabs() {
            ...
            $ul.append("<li class='active'><a href='#' class='previewTab'>Video Preview</a></li>");
        ...
        function showPreview() {
            ...
            $(".videoPreviewPane").empty().show();
            $(".embedForm").hide();
            $(".videoPreviewPane").showSpinner();

            $(".videoPreviewPane").video({
                mediaId: options.mediaItem.mediaId,
                apiRoot: APIRoot,
                showPreviewOnly : false,
                postProcess: function(){$(".videoPreviewPane").hideSpinner(); }
            });
        ...
        function showEmbedForm() {
            ...
            $(".videoPreviewPane").hide();
            $(".embedForm").show();

            if ($(".embedForm").html() === '') {// just load form once.

               $(options.target).find(".embedForm").load("templates/VideoEmbedForm.htm", function () { setupEmbedForm(); });
        ...
        var generateCodeBlock = function () {
            ...
            codeBlock += '<!-- Markup for CDC Video (' + options.mediaItem.title + ') -->\r';
            codeBlock += '<script src="' + urlRoot + '/Scripts/jquery-1.9.1.min.js" type="text/javascript"></script>\r';
            codeBlock += '<script src="' + urlRoot + '/Scripts/Marketplace.Video_0.1.js" type="text/javascript"></script>\r';
            codeBlock += '\r';
            codeBlock += '<div class="CDCVideo_' + options.mediaItem.mediaId.toString() + '"></div>\r';
            codeBlock += '<script language="javascript" type="text/javascript">\r';
            codeBlock += '$(".CDCVideo_' + options.mediaItem.mediaId.toString() + '").video({\r';
            codeBlock += '   mediaId: ' + options.mediaItem.mediaId + ',\r';
            codeBlock += '   apiRoot: "' + APIRoot + '",\r';
            codeBlock += '   showPreviewOnly: false\r';
            codeBlock += '})   \r';
            codeBlock += '</script>\r';
        ...
        var generateRawCodeBlock = function () {
            ...
            var codeBlock = '<!-- Markup for Video Content (' + options.mediaItem.title + ') -->\r';
            codeBlock += $(".videoPreviewPane").html();
            $preEmbed.text(codeBlock);
