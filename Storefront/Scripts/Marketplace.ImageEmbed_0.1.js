...
//IMAGE EMBED IS THE EMBED CODE (TAB) AND TABS SETUP

(function ($) {
    var PLUGIN_NAME = 'imageEmbed';
    ...
    $.fn[PLUGIN_NAME] = function (options) {
        ...
        function main() {
            ...
            var $divPreview = $("<div class='imagePreviewPane'>");
        ...
        function setupTabs() {
            ...
            $ul.append("<li class='active'><a href='#' class='previewTab'>Image Preview</a></li>");
        ...
        function showPreview() {
            ...
            $(".imagePreviewPane").show();
            $(".embedForm").hide();
            $(".imagePreviewPane").showSpinner();

            $(".imagePreviewPane").image({
        ...
        function showEmbedForm() {
            ...
            $(".imagePreviewPane").hide();
            $(".embedForm").show();

            if ($(".embedForm").html() === '') {// just load form once.

               $(options.target).find(".embedForm").load("templates/ImageEmbedForm.htm", function () { setupEmbedForm(); });
        ...
        var generateCodeBlock = function () {
            ...
            codeBlock += '<!-- Markup for CDC Image (' + options.mediaItem.title + ') -->\r';
            codeBlock += '<script src="' + urlRoot + '/Scripts/jquery-1.9.1.min.js" type="text/javascript"></script>\r';
            codeBlock += '<script src="' + urlRoot + '/Scripts/Marketplace.Image_0.1.js" type="text/javascript"></script>\r';
            codeBlock += '\r';
            codeBlock += '<div class="CDCImage_' + options.mediaItem.mediaId.toString() + '"></div>\r';
            codeBlock += '<script language="javascript" type="text/javascript">\r';
            codeBlock += '$(".CDCImage_' + options.mediaItem.mediaId.toString() + '").image({\r';
        ...
        var generateRawCodeBlock = function () {
            ...
            codeBlock += $(".imagePreviewPane").html();
            $preEmbed.text(codeBlock);
